...
`git diff --name-only origin/main...HEAD` rather than from the last commit — the way v5.5 got it
wrong. Two production files: `scripts/lib/brief-shape.mjs` (new, whole) and the range
`scripts/lib/vault-write-guard.mjs:157-200` (the second question the guard now asks).

**98.86 % — 173 killed, 2 survived, 0 timeout.** `brief-shape.mjs` **98.73 %**, the write guard's
changed range **100.00 %**. Four passes, and the three corrections are the interesting part:

| Pass | Score | Survivors | What it took |
|---|---|---|---|
| 1 | 71.84 % | 49 | the rules were asserted, the READING of a note was not |
...
survived. What survived was everything that decides *what the note says*: whether a ``` mentioned
inside a bullet opens a fence (it must not — the fold would stop being detected and the cap would
silently stop counting), whether a `#` inside a bullet is a heading (read as one it resets the whole
first screen and loses what came before), whether a blank line ends a bullet, whether an indented
marker is a second bullet or nesting folded into its parent. Thirteen poles, no production change:
the behaviour was already right, none of it was observed.

**Pass 1 also measured a duplication out of existence.** Twelve survivors sat on a second copy of
the bullet regex, in the verdict rather than in the reader — two spellings of *"what is a bullet"*.
They are gone because the two answers now come out of one walk (`readFirstScreen`), not because a
test was written for the copy.

**The messages are the product, and six string mutants proved nothing was asserting them.** Every
sentence's advice half — *"move it below the first `##` as ammunition, where nothing is counted"* —
could be emptied with the suite green. Its reader is not a human scanning a lint report: it is the
model that has to fix the note in one pass without seeing the file. The four refusals are now
asserted verbatim, singular and plural, including the space that separates two of them.

**Pass 3 → 4 — the shape this repo keeps meeting: a fail-open written twice.** The write guard's new
code scored 82.35 %, and five of its six survivors were one construct — a `try`/`catch` and a chain
of `?.` around a parse that had already succeeded two lines above. No test can reach those branches,
and none should be written for them: the precondition is the point, and the shell that catches the
genuinely unexpected is `runGuard`, fail-open by construction. Deleted, the range goes to **100 %**.
Same lesson as the v5.1.0 duo run, on a file written a month later.

**The 2 left are equivalents, both on the bullet marker** `/^([-*+]|\d+[.)])\s+(.*)$/`: dropping the
`$` where `(.*)` is greedy and there is no `m` flag, and narrowing `\s+` to `\s`, where the
...
Scoped to the one file the hotfix changes, `scripts/lib/declared-spellings.mjs`, measured whole at
`9602b54`.

**94.70 % — 122 killed, 7 survived, 3 timeout**, up from **93.94 %** on the first pass at `326e7ea`.

**Every mutant on the new span dies.** The entry added is `/\[\[[^\]\n]*\]\]/g`, and the four tests
that pull it are one per property rather than one per example: the reported repro (the target comes
back unchanged and the hit is reported instead of corrected), the alias half of `[[target|alias]]`
(spared deliberately), the prose on **both** sides of a wikilink (protection is a span, not a licence
to stop working for the rest of the line), and an unclosed `[[` (the span must not run past the line —
the mutant that drops `\n` from the character class dies on that one alone).

**The one killable survivor, and it was pre-existing.** `\s*` → `\S*` between `never` and the colon
survived the first pass: with it, `- aXiom — never : Axion` yields the spelling `': Axion'` instead of
`'Axion'`, so the matcher looks for a string no note contains. Nothing asserted the space **before**
the colon, which is exactly how French is typed — the owner's own language. Closed by one test, **no
production change**: the behaviour was already right, it was merely unobserved.

**The 6 left are equivalents, and they are all of two shapes already named in the retrospective.**

- **Anchors and greediness that nothing can distinguish** (3, all on the entry pattern at line 36):
  `$` dropped where `(.+)` is greedy and the input is one line; `^` dropped where the leading
  quantifier is lazy, so the leftmost match still starts at 0; `\s+` → `\s` before the wrong-spelling
  list, where the reader `.trim()`s every item afterwards. Same for `/^>.*$/gm` → `/^>.*/gm` at line
  129: with `m` set and `.` not matching a newline, the anchor changes nothing.
- **A fallback whose planted value cannot reach an observable output** (3): `[]` → `["Stryker was
  here"]` at lines 104, 105 and 136. In each case the planted string is filtered out one step later —
  a string destructured as `{ canonical, wrong }` yields two `undefined`s, a rule whose `to` is not a
  string is dropped by the very next `.filter`, and a span destructured as `[from, to]` compares
  characters against offsets, so `at < to` is false for every offset. Killing any of them would mean
  asserting on the literal a mutant invents, not on behaviour.

## v5.5 — the figure v5.1.0 told its readers was owed — 2026-09-13

**This run pays a debt that was made public.** The v5.1.0 release note says, in as many words, that
every figure in it was *"measured on 3 September, on the code as it then stood"* — before a review
whose six fixes changed four of the files those runs covered. So the numbers it published were the
score of the work, not of the bytes the tag shipped, and the note told readers so rather than hiding
it. The re-measurement was deliberately not run then (the instrument itself was being changed), and
its **when** was set by the owner for this release: *« on pourra aussi faire tourner le mutation
testing juste avant de cut la release pour fermer le 113 »*.

**The six targets, measured WHOLE** — 946 mutants, **96.09 %** overall (906 killed, 37 survived, 3
timeout), **16 minutes** behind the narrowed instrument. The five debt files, plus `lib/yaml-scalar.mjs`,
which the review ADDED and which had never been measured at all.

| Target | This run | After | Survivors left |
|---|---|---|---|
| `lib/source-key.mjs` (whole) | **99.43 %** | — | 1 |
| `lib/filed-note.mjs` (whole) | **98.10 %** | — | 4 |
| `lib/remote-sync.mjs` (whole) | **95.91 %** | — | 9 |
| `lib/remote-arrivals.mjs` (whole) | **95.57 %** | — | 9 + 1 timeout |
| `dated-note-path.mjs` (whole) | **95.40 %** | — | 4 + 2 timeout |
| `lib/yaml-scalar.mjs` (new, whole) | **80.39 %** | **100.00 %** | **0** |

⚠️ **These are not the same measurement as the figures they replace, and the table must not pretend
otherwise.** The earlier runs measured **hunks** — `lib/remote-sync.mjs (150-176)`, `dated-note-path.mjs
(45-137)` — because that is what those releases changed. This one measures each file **whole**, so it
reaches code no previous pass ever put under a mutant. A whole-file number below a hunk number is not
a regression; it is a wider question. What the debt asked for was an honest figure for the shipped
bytes, and that is what the column above is.

### 🔴 The finding: the file the review ADDED was the one nobody had measured

`lib/yaml-scalar.mjs` decides whether a value going into a note's header needs quoting — it exists
because `author:` is stamped from `git config user.name`, which a person may set to `@tpierrain`, a
character YAML reserves. It scored **80.39 %**, the lowest of the six, and **all ten survivors were
one shape**: every test proves what *gets* quoted, and not one proves what does not.

That is the expensive half, and the source file says so itself: *"quoting everything would pass every
test here while rewriting the header of every note in every brain"*. The mutants are precisely the
loosenings that would do it:

- **Drop the anchors on the boolean list** and `London` is quoted, because it ends in `on`. So is
  `Bruno`. YAML's `yes`/`no`/`on`/`off` are whole values, never substrings.
- **Let the number match a prefix** and `12 Monkeys` becomes a number; **let it match a suffix** and
  `Agent 0x1f` does. Both are names.
- **Forbid the sign inside an exponent** and `1e+5` stops being a number — the lie the quoting exists
  against, reintroduced by removing two characters.

**Eleven assertions, each the killer of one survivor, and no production line changed** →
**100.00 %, zero survivors.** The values chosen are real ones (`.NET`, `12 Monkeys`, `London`),
because a boundary case that could never be typed teaches the next reader nothing.

### The other five: the survivors are the known set, and one thing they are not is new

Cross-read against this file's own history rather than re-adjudicated from scratch:
`filed-note.mjs:37` ×2 and `:257` are the equivalents proved in the 2026-09-05 duo run (the slug
collapses runs of non-alphanumerics, so the anchors are free), and `filed-note.mjs:114` is the one
that run recorded as **not** an equivalent, with its verdict already written: *"the honest answer is
probably a guard on an undeclared tier rather than a test of one. No production change is owed."*
That verdict still holds.

🔭 **What this run does NOT claim.** The survivors in `remote-sync.mjs`, `remote-arrivals.mjs` and
`dated-note-path.mjs` that fall **outside** the previously-measured hunks have not been adjudicated
one by one — they are the price of widening from hunks to whole files, and calling them equivalents
without reading them is exactly the shortcut this register exists against. They are recorded as
**unexamined**, not as accepted. The debt that was public was the *figure*, and the figure is now
honest.

## v5.5 gate 1 — what the branch itself wrote, and the payment that was WRONG the first time — 2026-09-13

**This is the release's own gate, and it is not the run above.** §9bis gate 1 asks for the mutation
score of *what this branch writes*; the section above pays an old public debt about files v5.1.0
shipped. Two passes, two subjects, and conflating them is how one of them goes missing.

🛑 **It was first recorded as paid on ONE file — and the branch writes six.** The entry read *"this
branch writes one production file, `lib/batched-announcement.mjs`"*, measured it (17 mutants,
**100 %**), and closed the gate. The five files of #115 — `engine-fingerprint-table`, `engine-source`,
`engine-heal`, `reconcile-brain`, `staged-skills`, **+181 lines** — are the fix this release *is*, and
none of them had been under a mutant. The cause is mechanical and worth more than the score: the claim
was written from the **last commit** instead of from `git diff --name-only origin/main...HEAD`. A gate
whose scope is "what the branch wrote" must be read off the branch.

⚠️ **And it is a worse shape than the v5.3.0 skip this file already records.** That one left *nothing*
— no log, no number, no trace. This one left a **green log with a real score in it**, which reads as
coverage from every angle except recomputing the file list.

**Re-run hunk-scoped over all six, 2026-09-13 — 103 mutants, 100 killed, `97.09 %`, and every survivor
is an accepted equivalent.**

| Target | Mutants | Score | Survivors |
|---|---|---|---|
| `lib/batched-announcement.mjs` (whole, new export) | 17 | **100.00 %** | 0 |
| `lib/engine-fingerprint-table.mjs:19-110` | 36 | **97.22 %** | 1, equivalent |
| `lib/engine-heal.mjs:37-64` | 14 | **92.86 %** | 1, equivalent |
| `lib/engine-source.mjs:35-52` | 13 | **92.31 %** | 1, equivalent |
| `lib/reconcile-brain.mjs:177-182,313-331` + `lib/staged-skills.mjs:49-103` | 23 | **100.00 %** | 0 |

Logs: `reports/mutate-one-v55-branch.log` (the first, partial payment),
`reports/mutate-one-engine-fingerprint-table-19-110+4.log` (the wide re-run),
`reports/mutate-one-v55-branch-narrow.log` (the two files re-scoped, below).

### The three survivors, and why each cannot be killed by a test worth writing

- **`engine-fingerprint-table.mjs:34` — the `$` dropped from `/^templates\/([^/]+)\/(.+)$/`.** `(.+)`
  is greedy and `.` does not cross a newline, so the two regexes differ **only** on a path containing
  a literal `\n`. The input is a list of git-tracked source paths; the fixture that would kill this
  mutant is one no tree can produce.
- **`engine-heal.mjs:53` — the sort comparator `a.rel < b.rel` widened to `<=`.** It changes the order
  only for two entries with the **same** rel, and the rels are object keys: equal ones cannot exist.
- **`engine-source.mjs:48` — `(manifest?.retired ?? [])` given a non-empty default.** The injected
  value is a tombstone glob that matches no rel in any manifest, so it retires nothing. Only a file
  literally named after the mutant's own string could tell the two apart.

### 🔍 What the WIDE window found is pre-existing, and it is named rather than quietly dropped

The first re-run used generous ranges (`reconcile-brain.mjs:177-331`, `staged-skills.mjs:21-103`) and
scored **94.57 %** with seven survivors. Four of them sit on lines **this branch never touched**, so
re-scoping to the changed lines is what §5quinquies prescribes — *on a hunk-scoped run, every survivor
is yours* — and the re-scoped run is the 100 % row above. The four are **not** thereby adjudicated:

- **`reconcile-brain.mjs:256` — `retireShippedWorkflows({ brainDir, sourceDir })` emptied to `{}`.**
  Nothing observed the difference, which says the retirement's *result* is unasserted on this path.
  The most interesting of the four.
- **`reconcile-brain.mjs:280` and `:285`** — the install-if-absent branch (the regex that trims a
  skill glob, and the `utf8` of the read that seeds the new skill's provenance base). Both live in a
  loop that only runs when a brain is **missing** an engine skill.
- **`staged-skills.mjs:44`** — the `utf8` of the staging read inside `readStagedProvenance`.

Recorded as **pre-existing and unexamined**, never as accepted equivalents. They belong to the harness
test-quality chantier, not to this release, and they are written into its plan rather than left in a
log nobody re-reads.
