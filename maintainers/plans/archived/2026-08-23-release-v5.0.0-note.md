...
> ## ✅ PUBLISHED _(2026-08-23 · archived the same day)_
>
> The body below the first `---` is what was published, verbatim, at
> <https://github.com/tpierrain/kenjaku/releases/tag/v5.0.0>. The drafting box that follows this line
> never reached a reader: publishing strips everything above that separator.
>
> **Two things were repaired in the body on the eve of the cut**, and they are the reason a release
> note is re-read rather than trusted: its suite figure said 2 596 on a suite that runs 2 707, and it
> said nothing about the field rehearsal — the only check that exercises what every reader lives,
> their own installed engine fetching the release. The issue list at the end was added after the tag,
> on the owner's ask, once the tracker had been swept (`CONVENTIONS.md` §10bis).
