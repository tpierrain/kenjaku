...
1. **A brain open on two machines stays in step on its own** ([#84](https://github.com/tpierrain/kenjaku/issues/84)). Until now a brain caught up only when a conversation started — a deliberate choice, since a brain that reaches for the network mid-work can interrupt it. The cost landed elsewhere: a window left open all afternoon kept answering from what it knew that morning. A clock inside the search server now pulls on an interval, only on a clean tree, one machine's window at a time, and the arrival is announced to the person at their next message (plus a native banner when someone else wrote it).
2. **Two people on one brain stop storing the same thing twice** (the duo-mode sub-plan, landed in `415cd7c`). A captured source carries an identity derived from what it *is* rather than from who fetched it, the writer guard refuses a known duplicate on the deterministic path, and per-person paths keep two syntheses of one day from colliding.
