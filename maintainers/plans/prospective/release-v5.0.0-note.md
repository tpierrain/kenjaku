...
> ### v5.0.0 ends the choice between keeping the changes you made to your second brain's capabilities, and receiving the improvements that follow.
...
- **On a French brain, one skill arrives in English.** The retired `tdd-discipline` had a French twin; its successor `test-first-discipline` ships in English only. Your brain reads English natively, so nothing stops working — and the skill only ever loads when you ask your brain to *write or change code*, so a brain used for notes never meets it. What the French text said, in any case, was the rule this release retired: *baby steps, not test-first batches*, which was measured and did not hold up. The English successor is the one that did.
...
- Full suite green: **2 707 tests, 2 704 passing, 3 skipped, none failing**, plus 87 maintainer-side checks. Mutation is measured per block and scoped to the change, with every survivor either killed or documented as equivalent — figures in `maintainers/mutation/RESULTS.md`.
- **The update was rehearsed on copies of real, installed brains before this tag was cut** — not on fixtures: a brain running v4.9.1 and another running v3.5.0, each copied, each updated by *its own installed engine* fetching this release, which is the one path no test can exercise. Both unfroze on the first update, and the owner's territory came out byte-identical: notes, constitution, `.env`, settings the engine does not own. That trial is also what caught the release's last serious defect, weeks of work in: the first update landed nothing and reported success.
