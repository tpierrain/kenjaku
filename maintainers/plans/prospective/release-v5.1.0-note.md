...
- 📖 **And what sharing a brain does — and does not — do is written down.** The short version: it shares what you wrote down, not what you can see in your own tools. Your mail, your messages, your calendar stay yours; a shared calendar is the one thing that crosses over. [The duo-mode page](https://github.com/tpierrain/kenjaku/blob/main/docs/duo-mode.md) has the whole thing — what each person does, who controls access and how a duo ends — and is worth two minutes before you invite anyone in.
...
*One last thing, since it is about your notes.* Before tagging, we reviewed this release whole one more time and found six defects in it — three of which could, in the wrong circumstances, have cost someone a note. All six are fixed here, each with a test that fails on the old code, and none of them was ever in a version anyone is running: they were in the new code, and they never left the workshop. The detail is below, for whoever wants it.
...
**The review before the tag, and the six things it found.** A full review of the release's own code, run once the work was green, turned up six defects and held the tag until every one was fixed. Three could have cost a note. **The catch-up's undo** reset the tree to `ORIG_HEAD`, a ref a no-op rebase leaves naming an arbitrary older commit — so an integration that failed could have deleted notes written since; it now works from a SHA the tick reads itself, and checks that the undo actually ran before announcing that it did. **The freshness probe** compared the remote against the tracking ref its own `fetch` had just advanced, so one failed integration silenced every later tick for good; it now asks whether that commit is already contained in `HEAD`, and matches `refs/heads/<branch>` line by line rather than letting `ls-remote` glob-match a sibling. **The `author:` stamp** interpolated `git config user.name` into YAML raw, so a name opening on a YAML indicator (`@tpierrain`, `- tp`) made every note that machine filed unparseable — and, on a shared brain, tripped the partner's header check and undid their whole pull. It is now quoted only where quoting is needed, in all three places that produce that line: the writer, the tool that prints it to be pasted, and the instructions that ask for it. The remaining two: a source key our own documented spelling wrote in quotes while the duplicate check compared it unquoted — so the notes produced from those very instructions were invisible to the check, and the reader now unwraps a wrapping quote pair, which is what makes them findable again — and a French em dash in shipped prose.
...
**Quality.** Mutation-tested on every half of what this release writes: the search-server half **82.89 % → 97.37 %**, the harness half **97.22 %** and **82.14 % → 100 %** across two batches, and the duo-mode question — the part the release was held for — **84.60 % → 98.92 %** on the three files it adds, **92.02 % → 95.45 %** on what it changed elsewhere. And the safeguards a duo owes the person whose brain it is, measured whole across four batches: **97.76 %**, **96.32 %**, **100 %** and **95.98 %** — closed by four new tests, with not one line of working code changed. Every survivor left is a named equivalent. **Every figure above was measured on 3 September, on the code as it then stood** — before the review described just above, whose six fixes changed four of the files those runs covered (`remote-sync.mjs`, `filed-note.mjs`, `dated-note-path.mjs`, `source-key.mjs`) and added tests to each. They are therefore the score of the work, not of the exact bytes this tag ships; the re-measurement is owed and is deliberately not run here, because the instrument itself changed mid-release (it now judges a target with the tests that can see it, and that change has its own proving run still open). The whole test suite, by contrast, is green on the shipped bytes: 3226 tests. The findings are in `maintainers/mutation/RESULTS.md`, newest-first — including a guard whose three riders no test ever saw apart, and the mutation runner's own refusal of a measurement it had really made. Documentation follows: ADR 0011 gained the trigger row for this clock, and the marketing surface was re-read whole (§10) — one absolute promise this release turned into a half-truth, *"you share the generator, never the brain"*, corrected where it appeared. **It was then re-read a second time, against the six fixes, and no page moved**: what those fixes restore is exactly what the pages already promised, so the code came to the page rather than the page to the code. Three decisions gained a precision instead (ADR 0011 on which reference "already has it" means and on an undo that can fail, ADR 0041 on why the key reader unquotes, ADR 0034 on the four universe names a YAML parser reads as booleans), and the one real correction was not on a marketing page at all: the instructions a brain follows to stamp who wrote a note still taught the raw spelling.
