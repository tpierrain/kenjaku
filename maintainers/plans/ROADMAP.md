...
| `prospective/field-findings-2026-08-02-action.md` | 16 findings from one evening on a real brain, cut into three releases: **v4.5.0** promises kept (multi-machine clone, failure ≠ wait, disk↔index crosscheck), **v4.6.0** the vault's identity (resolve before writing, homonymy), **v4.7.0** visibility (banner, offers, target version, commit messages). | ahead of 3 | 🟢 **ACTIVE.** **v4.5.0 SHIPPED** (2026-08-03, tag `v4.5.0`, PR #54, CI 7/7, merge `96f5999`) **and v4.6.0 SHIPPED** (2026-08-04, tag `v4.6.0`, PR #55, CI 7/7, merge `c0b2b16`) — each release's published note and PR body archived beside it (`archived/release-v4.5.0-note.md`, `archived/release-v4.6.0-note.md`), mutation numbers pinned in `mutation/RESULTS.md`. The plan stays **prospective**: it still owns **v4.7.0** (visibility). Resume at `## Step 13`, whose first step is a **scope call with the owner** (the findings are listed, the cut is not decided). The freeze trap runs alongside; its "path back upstream" half leaves for its own ADR. |
