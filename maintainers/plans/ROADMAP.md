...
| `prospective/field-findings-2026-08-02-action.md` | 16 findings from one evening on a real brain, cut into three releases: **v4.5.0** promises kept (multi-machine clone, failure ≠ wait, disk↔index crosscheck), **v4.6.0** the vault's identity (resolve before writing, homonymy), **v4.7.0** visibility (banner, offers, target version, commit messages). | ahead of 3 | 🟢 **ACTIVE.** **v4.5.0 SHIPPED** (2026-08-03, tag `v4.5.0`, PR #54, CI 7/7, merge `96f5999`) **and v4.6.0 SHIPPED** (2026-08-04, tag `v4.6.0`, PR #55, CI 7/7, merge `c0b2b16`) — each release's published note and PR body archived beside it (`archived/release-v4.5.0-note.md`, `archived/release-v4.6.0-note.md`), mutation numbers pinned in `mutation/RESULTS.md`. The plan stays **prospective**: it still owns **v4.7.0** and now **v4.8.0**. **v4.7.0 SHIPPED too** (2026-08-05, tag `v4.7.0`, PR #57, CI 7/7, merge `556f950`, note + PR body archived beside the others) — deliberately **short** by the owner's 2026-08-05 scope call: that morning's field session only (the machine that is behind and never says so, the index promising a resume it cannot know is coming, `/univers` not being a command) plus **PR #56**, our dependency pins. The rest of the visibility list (banner offers, target version, frozen source, commit messages, the always-loaded instruction layer) moved to **v4.8.0** with its entries untouched. **v4.8.0's scope was decided the same day, and it is short again**: F3 (the engine-update offer names no target version and cannot say what the jump contains) plus its sibling, the version-blind SessionStart offer — everything else keeps waiting. **Resume at `## Step 14`, box 14.1** (cut the branch). The freeze trap runs alongside; its "path back upstream" half leaves for its own ADR. |
