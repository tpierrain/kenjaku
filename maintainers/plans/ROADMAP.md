...
| `prospective/field-report-2026-08-07-rag-orphans-and-windows-launchers.md` | The RAG MCP server never exits when the client disconnects, so orphans pile up on the SQLite lock until a session silently has **no vault tools at all**; plus two Windows launcher defects (LF-only `.cmd`, `npx tsx` resolving through the registry) that pushed startup over the 30 s ceiling. | **ahead of everything** | 🔴 **LIVE — v4.8.1 hotfix, owner's decision 2026-08-07 (ahead of both v4.9.0 subjects). OPEN** — runtime report from two Windows colleagues on v4.6.0, **re-verified against HEAD**: all three still live, and defect 1 is cross-platform and self-aggravating. Argues for a **v4.8.1 hotfix** before the v4.9.0 subjects. Source report archived as `archived/field-report-2026-08-07-source.md`. |
| `prospective/active-universe-follows-the-owner-action.md` | The active universe travels with its owner, and the machine that pulls a switch lands on it cleanly. | ahead of 3 | 🔴 **LIVE — release #1, in flight.** S1 + S2 done on `feat/active-universe-travels`; **S3 next, leading with the ordering defect** (the arriving machine announces the universe it is leaving), then the fleet `.gitignore` migration, then S4 docs, then cut. Scope arbitrated 2026-08-08: **universes only**. |
| `prospective/update-regime-owns-what-it-shipped-action.md` | The engine updates what it shipped and only stops for what the owner really wrote: an immutable base, a real three-way merge, a write guard, an audible divergence, and the doctrine layer finally joining a regime. | supersedes 4's F-B7e | 🟡 **NEXT, and it is the big one** _(sequenced 2026-08-08, starts once release #1 is cut)_. Absorbs the silent-skill-freeze finding as a symptom and carries the source-first rule as its demonstration. `mind-palace` stays **frozen on purpose**: it is the acceptance test, and the replayable fixture is rebuilt from public tags. |
