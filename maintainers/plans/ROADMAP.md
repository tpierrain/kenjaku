...
| `prospective/field-finding-2026-08-08-source-first-and-frozen-doctrine.md` | A source the owner hands over (URL, path) must be read before any search tool, and the doctrine layer that says so must actually reach deployed brains. | ahead of 3 | 🟢 **OPEN (2026-08-08)** — measured, not supposed: a brain pinned at **v4.8.1** carries a `CLAUDE.engine.md` frozen at **install day**, 12 doctrine commits behind, because that file is in **no regime**. The rule is one paragraph; **S2 (which carrier) is an owner arbitration** and gates the work. Rides with the universes release by the owner's call. |
