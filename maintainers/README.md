...
- **`plans/`** — implementation plans. **The way in is [`plans/ACTIVE.md`](plans/ACTIVE.md)**: one door
  ...
  **past · present · future** axis:
  - **root of `plans/` = present** — action plans **mid-flight**. **This list does not carry their
    ...
    - [`link-syntax-agreement-guard-action.md`](plans/link-syntax-agreement-guard-action.md)
      — the write guard and the link checker must agree on what a `[[link]]` is, and a test says so.
