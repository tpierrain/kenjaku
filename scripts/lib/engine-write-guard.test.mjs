...
test("the brain directory itself is not a file to rule on", () => {
  // `relative(dir, dir)` is the empty string, which passes both the `..` and the
  // `isAbsolute` tests and would otherwise be handed on as a relative path.
  assert.equal(brainRelative({ filePath: BRAIN, brainDir: BRAIN }), null);
});
...
// ── the four sentences the owner reads, in full ────────────────────────────

test("THE SENTENCES ARE THE DELIVERABLE, so they are pinned whole, not sampled", () => {
  // Matching a phrase leaves most of a sentence unjudged — measured: six string
  // mutants emptied a clause each and every `assert.match` above stayed green.
  // These four strings ARE the product of this slice, so changing one must cost
  // a deliberate test edit, exactly like the update report's wording.
  assert.equal(
    decide("scripts/auto-commit.mjs").reason,
    "scripts/auto-commit.mjs is an engine file, and this brain merges it: your edit is kept, and " +
      "every future update three-way-merges it with the new version — a clash is reported to you, " +
      "never written. If what you want is to change how your brain behaves, CLAUDE.md is the layer " +
      "built for that, and a skill of your own costs nothing to carry.",
  );
  assert.equal(
    decide("scripts/lib/glob-match.mjs").reason,
    "scripts/lib/glob-match.mjs is an engine file this brain does not merge: the next engine " +
      "update will overwrite it with the new version, without a word, and your change will be gone. " +
      "If you mean to change how your brain behaves, write it in CLAUDE.md, or in a skill of your " +
      "own — those are yours, and no update touches them.",
  );
  assert.equal(
    decide("rag/launch.sh").reason,
    "rag/launch.sh is regenerated by the engine from a template: the next update writes it again " +
      "from scratch and your change will be gone. If you mean to change how your brain behaves, " +
      "CLAUDE.md is the layer built for that — a launcher is machinery, not doctrine.",
  );
  assert.equal(
    decide(".engine-base/CLAUDE.md").reason,
    ".engine-base/CLAUDE.md is the engine's record of the version your files came from — the engine " +
      "maintains it, and nothing else should write here. Changing it does not change your brain: it " +
      "makes the next update believe you never edited the real file, so your own edit is silently " +
